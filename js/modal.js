...
const test = async function (page, index, url) {
  let response = await fetch(url);
  let data = await response.json();
  let imageUrl = data.results[index].image_url;
  
  document.getElementById("movie"+page+index).setAttribute("src",imageUrl);
  films.push(imageUrl);
  return imageUrl;
}
for (i=0 ; i<5 ; i++) {
  test(1, i, 'http://127.0.0.1:8000/api/v1/titles/?sort_by=-imdb_score&page=1')
  } 
  for (i=0 ; i<3 ; i++) {
    test(2, i, 'http://127.0.0.1:8000/api/v1/titles/?sort_by=-imdb_score&page=2')
    }
